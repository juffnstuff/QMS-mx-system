...
async function main() {
  ...
  await prisma.maintenanceSchedule.createMany({
    data: [
      {
        equipmentId: granulator.id,
        title: "Blade inspection and sharpening",
        frequency: "weekly",
        nextDue: futureDate(3),
        lastDone: pastDate(4),
      },
      {
        equipmentId: granulator.id,
        title: "Full bearing replacement check",
        frequency: "annual",
        nextDue: futureDate(60),
        lastDone: pastDate(305),
      },
      {
      ...
      {
        equipmentId: press1.id,
        title: "Hydraulic fluid level check",
        frequency: "weekly",
        nextDue: futureDate(2),
        lastDone: pastDate(5),
      },
      {
        equipmentId: press1.id,
        title: "Platen alignment verification",
        frequency: "quarterly",
        nextDue: futureDate(45),
        lastDone: pastDate(45),
      },
      {
        equipmentId: conveyor.id,
        title: "Belt tension and tracking adjustment",
        frequency: "monthly",
        nextDue: futureDate(12),
        lastDone: pastDate(18),
      },
      {
        equipmentId: forklift.id,
        title: "Daily safety inspection",
        frequency: "daily",
        nextDue: futureDate(1),
        lastDone: now,
      },
    ],
  });
    ...
    data: [
      {
        equipmentId: granulator.id,
        userId: operator1.id,
        description: "Replaced dulled granulator blades. Set #4 installed.",
        partsUsed: "Blade set #4 (6x cutting blades)",
        performedAt: pastDate(4),
      },
      {
        equipmentId: press1.id,
        userId: operator1.id,
        description: "Topped off hydraulic fluid. Level was slightly low.",
        partsUsed: "2 gallons AW-46 hydraulic fluid",
        performedAt: pastDate(5),
      },
      {
        equipmentId: conveyor.id,
        userId: operator2.id,
        description: "Adjusted belt tracking. Replaced worn idler roller on return side.",
        partsUsed: "1x idler roller (6\" diameter)",
        performedAt: pastDate(18),
      },
      {
        equipmentId: forklift.id,
        userId: operator2.id,
        description: "Daily pre-shift inspection completed. All systems normal.",
        performedAt: now,
      },
      {
        equipmentId: mixer.id,
        userId: operator1.id,
        description: "Noticed minor seal wear on rotor. Scheduled for follow-up service.",
  ...
  await prisma.workOrder.createMany({
    data: [
      ...
      },
      {
        ...
        description: "Drive belt showing signs of cracking. Replace before failure.",
        priority: "low",
        status: "open",
  ...
  console.log("  Admin:    admin@rubberform.com / admin123");
  console.log("  Operator: mike@rubberform.com / operator123");
  console.log("  Operator: sarah@rubberform.com / operator123");
}
